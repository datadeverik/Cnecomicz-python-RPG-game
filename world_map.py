...
character_creator_node_3 = db.Node(text="Your name is {name}.",
	responses={
		"A" : db.Response("Thank you. Goodbye.", 3, [character_creator.stop_talking, lambda: setattr(player, "can_move", True)]),
	}
...
guy1.dialogue_dict = {
	0 : db.Node("Yes or no?", { 
			"A" : db.Response("Yes.", 0, [guy1.stop_talking, lambda: setattr(player, "can_move", True)]),
			"B" : db.Response("No.", 0, [lambda: guy1.a_star_pathfind(-100,-100, BLOCKS), guy1.stop_talking, lambda: setattr(player, "can_move", True)]),
			"C" : db.Response("N/A.", 0, [guy1.stop_talking, lambda: setattr(player, "can_move", True)], False),
		}),
}